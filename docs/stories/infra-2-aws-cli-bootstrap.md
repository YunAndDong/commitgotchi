...
- 생성 대상:
  - **ECR** ×2: `commitgotchi-springboot`, `commitgotchi-fastapi` (Vue는 Chrome 확장이라 ECR 대상 아님)
  ...
  - **Elastic IP**: EC2에 EIP 할당(재시작에도 고정). 이 IP를 DNS A 레코드에 등록(아래).
  - **SSM 적재**: 생성 결과(prod 큐 URL, 버킷명, prefix 등) + secret(SecureString)을 `/commitgotchi/prod/...`에 적재
- **DNS(가비아, 수동):** AWS 리소스가 아님. EIP 할당 후 **가비아 DNS 콘솔에서 `commitgotchi.store` A 레코드 → EIP**를 사람이 등록한다(plan §6.8). Route53 전환은 ALB 도입 시(후속).
...
## 검증 / 보류
- ✅ `scripts/aws/bootstrap-infra-2.sh` 구현. 기본 plan 모드, `--apply --confirm-apply` 안전 게이트, `default` profile 차단.
- ✅ ECR×2, prod SQS+DLQ, 계정 접미사 S3 bucket, EC2 IAM role/profile, Security Group, EC2, EIP, SSM 적재 로직 구현.
- ✅ secret은 SSM `SecureString`으로 적재하고 로그/plan에서는 redacted 처리.
- ✅ `bash -n`, script plan mode, `aws sts get-caller-identity --profile commitgotchi-bootstrap`, EC2 `--dry-run`, default profile 차단 검증 완료.
- ✅ 실제 AWS 리소스 생성/변경 apply는 운영자 최종 승인 전까지 미실행.
- ⏭ 도메인(가비아 A 레코드), Certbot/TLS, compose 배포, SSM env fetch는 INFRA-3에서 진행.
