...
- ✅ `AWS_PROFILE=commitgotchi-bootstrap ./scripts/deploy.sh --plan`
  ...
  - ECR `prod` tag 이미지 2개가 없어 실제 deploy 전 build/push 또는 INFRA-4 필요:
  ...
  - Docker compose pull/up, certbot, 원격 EC2 명령, AWS 변경 없음.
...
- ⏭ 실제 EC2에서 `scripts/bootstrap-tls.sh --apply --email ... --expected-ip 54.116.84.198`
  실행 후 인증서 파일 존재 확인.
- ⏭ Spring Boot/FastAPI ECR `prod` tag push 또는 INFRA-4 image build/push 완료.
