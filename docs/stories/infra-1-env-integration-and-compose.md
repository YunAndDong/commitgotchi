...
story: INFRA-1
status: draft
scope: infra (.env 통합 + 백엔드 통합 실행 + prod compose overlay 드라이런, API 전용 Nginx)
phase: Phase 2
...
related_files:
  ...
  - .env.prod.example          # 신규
  - docker-compose.yml
  - docker-compose.prod.yml     # 신규 (overlay, vue 제외)
...
- Vue 소스·로컬 개발은 유지: **로컬 `docker-compose.yml`의 vue 서비스는 개발/미리보기용으로 둘 수 있다.** 단 prod overlay에는 포함하지 않는다.
...
- **When** `docker compose -f docker-compose.yml -f docker-compose.prod.yml --env-file .env.prod up`,
- **Then** Spring이 `prod` 프로필로 부팅하고(`CORS_ALLOWED_ORIGINS`에 HTTPS origin 없으면 fail-fast로 부팅 실패하는 것을 확인), **vue 없이** Nginx 443이 `/api/**`·`/character-assets/**`에 응답한다(`/`는 프론트 없음).
...
- report consumer 워커 배선은 **INFRA-0**이 선행(별도 프로세스 + 별도 compose 서비스). 이 스토리는 그 worker 서비스를 prod overlay에도 포함할지 결정.
