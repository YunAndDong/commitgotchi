...
- `fastapi/docs/problem-bank-implementation.md`
...
다음 story로는 API/계약 연결을 별도로 다루는 것이 좋다.

권장 순서:

1. 내부 함수 출력 shape를 Spring Boot 콜백 계약과 최종 대조한다.
2. 일일 리포트 처리 파이프라인에서 `build_daily_report_recommendation()` 호출 지점을 만든다.
3. 실패 시 `recommendedQuizzes: []`를 보장하는 fallback을 계약 테스트로 고정한다.
4. 이후 필요할 때 API 엔드포인트나 SQS/Spring Boot 콜백에 연결한다.
