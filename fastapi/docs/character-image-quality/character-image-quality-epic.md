...
이 epic의 목표는 **거르기를 "규격에 맞게 교정하기"로 바꿔, 흐름 C 계약(§4.4/§7.6)에 맞는 2×3 투명 PNG 스프라이트시트를 한 번에 안정적으로 산출**하는 것이다. 프런트가 `spriteMeta`로 셀을 잘라 쓰는 계약을 깨지 않는 게 전제다.
...
| 3 | 셀 정렬 미검증 | `png_validation.py` | `width%3`,`height%2`,`프레임≥18px`만 확인. 셀 안에 스프라이트가 균일 정렬됐는지는 안 봄 → 프런트 슬라이싱이 깨질 수 있음 |
...
- **흐름 C 계약(§4.4/§7.6) 준수.** `POST /api/ai/commitgotchi` 요청/응답 shape, `spriteMeta`(columns=3, rows=2, frameMap, transparent), 6프레임 2×3 레이아웃을 유지한다.
...
계약 §7.6은 baby 16px / mature 18px로 **행마다 논리 셀 크기가 다르다.** 생성 모델은 6개 스프라이트를 픽셀 단위 균일 격자에 정확히 안 놓는다. 따라서 "한 장 생성 → 그대로 슬라이스"는 신뢰도가 낮다.
...
- **옵션 A(권장·계약 유지):** FastAPI가 6개 스프라이트를 감지·크롭·중앙정렬해 **균일 셀의 canonical 시트**로 재배치. `spriteMeta`는 기존 형태 유지(균일 셀 가정 성립).
...
- 후처리 결과가 흐름 C 계약의 2×3 레이아웃·`spriteMeta`와 정합한다(옵션 A) 또는 재협상된 계약과 정합한다(옵션 B/C, 팀 합의 시).
- 프런트가 `spriteMeta`로 6개 프레임을 깨끗하게 잘라 쓸 수 있다(셀 내 스프라이트 정렬 보장 또는 실측 박스 제공).
- 진짜 알파만 `READY`로 저장하는 안전규칙이 유지된다. 후처리/정규화 실패는 `FALLBACK`으로 안전하게 떨어진다.
- 품질 게이트(배경 투명 비율, 6셀 비어있지 않음 등)가 자동 검증된다.
...
**소비자 동작:** Spring Boot는 `FALLBACK`을 받으면 동일 2×3 레이아웃의 **기본 스프라이트 세트로 대체**하고 `image_status=FALLBACK`으로 표시한다(아키텍처 §4.4/§7.3). 향후 엔드포인트(`POST /api/ai/commitgotchi`, character-image epic Story 2)에서는 이 결과를 계약 §4.4의 `status="FAIL"` + `errorMessage`로 매핑한다. `fallback_reason`은 안전한 enum 코드라 시크릿/프롬프트 원문이 노출되지 않는다.
...
| `GRID_NORMALIZATION_FAILED` | 후처리 | 6스프라이트 2×3 분할 실패(붙은 스프라이트·빈 셀·과다 노이즈 밴드) |
