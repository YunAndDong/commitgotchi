...
"한 번에 제대로 됐다"를 정형적으로 보장한다. 알파 존재 여부만 보던 검증을 넘어, **배경 투명 비율·3셀 점유·셀 정렬** 같은 산출물 품질 게이트를 추가하고, 실제 Gemini로 도는 안전한 프로덕션 스모크 스크립트(메타데이터만 기록)로 파이프라인을 사람이 확인할 수 있게 한다.
...
- **품질 게이트 함수**(검증기와 별개, `READY` 직전 적용):
  ...
  - 3개 셀 각각에 불투명 스프라이트 픽셀이 존재(빈 셀 없음).
...
- 배경 투명 비율·3셀 점유 게이트가 합성 이미지에서 올바르게 통과/실패를 판정한다.
