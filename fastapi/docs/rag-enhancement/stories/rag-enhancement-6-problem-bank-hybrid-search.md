...
- [ ] `search_problem_bank()`에 optional embedding store/client와 가중치 인자를 추가한다.
- [ ] 키워드 점수와 벡터 유사도를 정규화/결합하는 deterministic 하이브리드 ranking을 구현한다.
- [ ] 임베딩 없음/실패 시 기존 키워드 결과와 동일하게 fallback한다.
- [ ] `recommend_quizzes()`가 하이브리드 후보 풀을 쓰도록 연결하되 snapshot shape와 dedupe를 유지한다.
- [ ] 의미 매치, fallback 동일성, 가중치, 추천 shape/dedupe, 결정성 테스트를 추가한다.
...
- Story 5의 problem embedding store가 선행되어야 한다.
- 추천 출력 contract와 최대 3개 limit는 변경하지 않는다.
- fallback 동일성은 기존 테스트 기대값을 보호하는 핵심 회귀 가드다.
