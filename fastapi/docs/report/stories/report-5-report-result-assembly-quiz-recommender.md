...
- Story 4는 Gemini 기반 `ReportAnalysis`를 만들고 `scoreDelta`, `statusMessage`, `dailyReport`, `nextRecommendation`을 validation/clamp/fallback으로 방어한다. `characterMetadata.emotion`은 입력 톤 컨텍스트이며 결과 payload field가 아니다.
...
2. Analyzer 결과 normalization
   ...
   - 최종 payload는 `status`, `scoreDelta`, `statusMessage`, `dailyReport`, `nextRecommendation`, `recommendedQuizzes`를 포함한다.
...
6. Fallback 처리
   ...
   - recommender가 예외를 던져도 analyzer가 성공했다면 `status`, `scoreDelta`, `statusMessage`, `dailyReport`, `nextRecommendation`은 가능한 범위에서 유지하고 `recommendedQuizzes`만 `[]`로 fallback한다.
...
2. 결과는 `status`, `scoreDelta`, `statusMessage`, `dailyReport`, `nextRecommendation`, `recommendedQuizzes`를 포함한다.
...
- analyzer failure는 primary failure다. `scoreDelta`, `statusMessage`, `dailyReport`, `nextRecommendation`의 근거가 없으므로 전체 `FALLBACK`으로 내려간다.
...
- analyzer 성공 시 `scoreDelta`, `dailyReport`, `nextRecommendation`, `statusMessage`는 Story 4 결과만 보존하고, recommender 입력은 추천 query 보조 신호로만 구성했다.
