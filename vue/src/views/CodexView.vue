...
watch([items, () => route.query.characterId], ([list, characterId]) => {
  ...
  const next = list.findIndex(item => item.owned && String(item.id) === String(characterId))
...
<template>
  <div class="codex col">
    <div class="codex-head row between">
      <div class="heading-left row">
        <RouterLink to="/select" class="codex-back cg-btn cg-btn--sm" aria-label="캐릭터 선택 화면으로 돌아가기">←</RouterLink>
    ...
    <div class="codex-layout">
      <section class="showcase cg-card col">
        <div class="flow cg-screen">
          ...
          <div class="track">
            <button v-for="(it, i) in items" :key="it.id" type="button" class="carousel-item"
                    ...
                    :aria-label="it.owned ? `${it.name} 선택` : '잠긴 커밋고치 선택'"
                    @click="idx = i">
        ...
        <div class="current center col">
          <strong class="big current-name">{{ current.owned ? current.name : '???' }}</strong>
          <p class="current-personality tiny muted">
            {{ current.owned ? (current.personality || '차분히 함께 성장하는') : '수집 후 성격 확인 가능' }}
          </p>
      ...
      <section class="reviews cg-card col">
        <div class="review-head row between">
          <div class="review-title col">
            <span class="cg-section-title">{{ current.owned ? current.name : '잠긴 커밋고치' }}</span>
        ...
        <div class="my-review review-summary">
          ...
          <RouterLink v-else-if="current.owned" :to="reviewWritePath" class="cg-btn cg-btn--primary cg-btn--sm">
            리뷰 작성
          </RouterLink>

          <RouterLink v-else to="/create" class="cg-btn cg-btn--accent cg-btn--sm">
            해당 커밋고치 키우기
          </RouterLink>
        </div>
...
<style scoped>
.codex { gap: var(--sp-4); width: 100%; max-width: 1040px; margin: 0; }
.codex-head {
  gap: var(--sp-3);
  align-items: center;
}
