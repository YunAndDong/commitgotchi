...
앱 시작 시 `<html data-theme="cozy">`로 고정하며 상단 내비에 테마 변경 버튼을 제공하지 않는다.
...
├── components/          # CgSprite·CgGauge·CgRadar·CgStatTile·CgEmo·CgConfetti·AppNav·Mascot
├── views/               # 화면별 컴포넌트(아래 표)
...
- **이미지 = Spring Boot ↔ FastAPI 동기 생성(흐름 C):** 생성 요청은 Spring Boot를 거쳐 FastAPI `POST /api/ai/commitgotchi`로 전달되고, UI는 PENDING/READY/FALLBACK 상태를 표시한다.
...
실제 1x3 스프라이트시트가 준비되면 이 컴포넌트만 교체하면 된다.
