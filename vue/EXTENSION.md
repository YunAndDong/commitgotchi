...
- 저장 데이터: `id`, `name`, `emotion`, `isEvolved`만 포함한다. 인증 토큰과 전체 게임 상태는 저장하지 않는다.
...
7. 새 일반 웹페이지 탭과 새로고침된 탭에도 저장된 활성 커밋고치가 표시되는지 확인한다.
8. 운영체제의 동작 줄이기 설정을 켜고 이동이 중지되는지 확인한다.
9. `chrome://extensions`에서 content script 관련 오류가 없는지 확인한다.
