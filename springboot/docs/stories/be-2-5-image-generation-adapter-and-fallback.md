...
3. **FastAPI 요청/응답 계약**
   ...
   - **And** `prompt`는 아키텍처 §7.6의 2x3 스프라이트 프롬프트에 `LearningCharacter.designKeyword`를 주입해 만든다.
...
4. **Fallback sprite 계약**
   ...
   - **And** `spriteMeta`는 FastAPI 성공 응답과 같은 2행 3열 `frameMap` 구조를 가진 JSON 문자열이어야 한다.
   - **And** fallback meta는 `baby/mature` 행과 `joy/sad/angry` 열 매핑을 제공한다.
...
- [x] **Task 1: 이미지 포트와 결과 모델을 추가한다** (AC: 2, 3, 4)
  ...
  - [x] 요청 모델은 `userId`, `characterId`, `designKeyword`, `s3ObjectUrl`, `prompt`를 내부적으로 표현하되, FastAPI DTO에는 `userId`, `s3ObjectUrl`, `prompt`만 직렬화한다.
...
- [x] **Task 2: 설정과 fallback meta를 추가한다** (AC: 1, 4)
  ...
  - [x] fallback meta는 FastAPI 성공 meta와 같은 2x3 구조를 중앙 factory에서 만든다.
...
- PRD/Architecture는 캐릭터 이미지를 단일 이미지가 아니라 진화 2단계 x 감정 3종의 2x3 투명 PNG 스프라이트시트로 정의한다.
...
- Architecture AD-12는 6프레임 2x3 스프라이트시트를 확정했고, AD-13은 이미지를 동기 HTTP 흐름으로 확정했다.
...
├── CharacterImagePromptFactory.java
...
파일명은 구현 관례에 맞게 조정 가능하다. 다만 `character.image` 패키지로 외부 HTTP DTO와 prompt/meta 생성 책임을 격리한다.
...
- 이미지는 6프레임 2x3 sprite sheet이며, 진화는 같은 sprite sheet의 mature 행 전환으로 표현된다.
...
- Response assertions:
  - `$.item.imageStatus`
  - `$.item.spriteSheetUrl`
  - `$.item.spriteMeta.frameMap.baby.joy[0]`
...
- GREEN/REFACTOR: Added image port, properties, prompt/meta factories, FastAPI `RestClient` adapter, image application service, creation/retry wiring, projection sprite fields, and API regression tests.
...
- Added centralized 2x3 fallback sprite metadata factory using `ObjectMapper` JSON serialization.
...
- `springboot/src/main/java/com/commitgotchi/character/image/CharacterImagePromptFactory.java`
